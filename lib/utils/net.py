...
def initialize_gpu_from_weights_file(model, weights_file, gpu_id=0):
    ...
    if 'cfg' in src_blobs:
        saved_cfg = yaml.load(src_blobs['cfg'], Loader=yaml.Loader)
    ...
    # debug: AssertionError: Workspace blob cls_score_w with shape (3007, 1024) does not match weights file shape (81, 1024)
    '''
    del src_blobs['cls_score_w']
    del src_blobs['cls_score_b']
    del src_blobs['bbox_pred_w']
    del src_blobs['bbox_pred_b']
    '''
